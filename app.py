...
TEMPLATE_SHEET = "schedule_template"
...
def ensure_current_initialized(sheet):
    current_ws = get_ws(sheet, CURRENT_SHEET)

    current_values = current_ws.get_all_values()

    if current_values:
        return False

    template_ws = get_ws(sheet, TEMPLATE_SHEET)
...
def calculate_result(df, previous_carryover):
    ...
    auto_place_count = min(
        shortage_after_missed,
        available_candidate_count
    )

    next_carryover = max(
        0,
        shortage_after_missed - available_candidate_count
    )
...
try:
    ...
    initialized = ensure_current_initialized(sheet)

    if initialized:
        st.success("schedule_template から current を初期化しました。")
    ...
    st.error("Google Sheetsの読み込みに失敗しました。")
    st.exception(e)
    st.stop()
    ...
    if not replacement_plan.empty:
        ...
        if st.button("選択した振替提案だけ反映する"):
            try:
                ...
                st.success(
                    f"{updated_count}件の振替提案を反映しました。"
                )

                st.rerun()

            except Exception as e:
                st.error("振替提案の反映に失敗しました。")
                st.exception(e)
...
with tab_input:
    ...
    st.write(
        f"選択中：{len(selected_indexes)}件"
    )

    if st.button(
        f"選択した時間を「{new_status}」に変更"
    ):
        try:
...
with tab_analysis:
    ...
    with col1:
        st.metric(
            "週最低勉強時間",
            f"{stats['weekly_required']}時間"
        )

    with col2:
        st.metric(
            "現在の勉強予定",
            f"{stats['actual_study_total']}時間"
        )
